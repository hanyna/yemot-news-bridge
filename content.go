...
// promoPhrases — "חתימה" בסוף הודעה שמזמינה להצטרף לערוץ (למשל "להצטרפות לערוץ
// הטלגרם של אלחנן גרונר"). זו לא פרסומת: ההודעה עצמה חדשות. לכן לא מוותרים על
// ההודעה — רק מורידים את החתימה מההקראה. (בעבר הביטויים האלה היו ב-adPhrases,
// וכמעט כל ההודעות של ערוץ שמוסיף חתימה כזו לא נכנסו לקו.)
var promoPhrases = []string{
	"להצטרפות לערוץ", "להצטרפות לקבוצה", "להצטרפות לעדכוני", "הצטרפו לערוץ", "הצטרפו לקבוצה", "הצטרפו עכשיו",
}

// promoTail: חתימה בסוף ההודעה — מה שאחרי הביטוי קצר מזה (תווים).
const promoTail = 150

// stripPromo מוריד חתימת "להצטרפות לערוץ..." מסוף ההודעה. found=true כשהורדה חתימה.
// ביטוי כזה באמצע הודעה ארוכה (לא בסוף) — נשאר כמו שהוא.
func stripPromo(text string) (out string, found bool) {
	for {
		cut := -1
		for _, p := range promoPhrases {
			i := strings.LastIndex(text, p)
			if i < 0 || len([]rune(text[i:])) > len([]rune(p))+promoTail {
				continue
			}
			if cut < 0 || i < cut {
				cut = i
...
// wasPromoAd: הודעה שהכלל הישן (חתימת הצטרפות = פרסומת) זרק. ראו promoSince.
func wasPromoAd(raw string) bool {
	for _, p := range []string{"הצטרפו לערוץ", "הצטרפו לקבוצה", "להצטרפות לערוץ", "להצטרפות לקבוצה", "הצטרפו עכשיו"} {
...
// promoSince: הודעות עם חתימת הצטרפות שפורסמו לפני התיקון (ההודעה האחרונה שנכנסה
// לשלוחה 1 לפניו — 25/09/2026 01:45) לא נכנסות עכשיו באיחור: הן היו מופיעות
// בראש השלוחה (מספר גבוה = נשמעות ראשונות) כאילו הן חדשות.
const promoSince = 1790290000
